...
public sealed class CatalogTechnologyService(LeanProdDbContext db) : ICatalogTechnologyService
{
        ...
        Guid id, SaveCatalogTechnologyCommand command, CancellationToken ct)
    {
        ...
        db.CatalogTechnologyStageLinks.RemoveRange(technology.StageLinks);
        ...
        SaveCatalogTechnologyCommand command, CancellationToken ct)
    {
        ...
        var existingStageCodes = (await db.CatalogTechnologyStages.AsNoTracking()
        ...
        var existingOperationCodes = (await db.CatalogTechnologyOperations.AsNoTracking()
            .Select(x => x.Code).ToArrayAsync(ct)).ToHashSet(StringComparer.OrdinalIgnoreCase);
        var nextStage = await db.CatalogTechnologyStages.CountAsync(ct) + 1;
        ...
        foreach (var stage in command.Stages)
        {
            if (!string.IsNullOrWhiteSpace(stage.Code))
                existingStageCodes.Add(stage.Code.Trim().ToUpperInvariant());
        ...
        var stages = command.Stages.Select(stage =>
        {
            ...
            return string.IsNullOrWhiteSpace(stage.Code)
                ? stage with
                {
                    Code = NextBatchCode(existingStageCodes, ref nextStage),
    ...
    private IQueryable<CatalogTechnology> QueryDetails() => db.CatalogTechnologies.AsNoTracking()
        ...
        .Include(x => x.StageLinks)
        .Include(x => x.Stages).ThenInclude(x => x.StageTemplate)
        .Include(x => x.Stages).ThenInclude(x => x.Department)
        ...
        SaveCatalogTechnologyCommand command, Guid? id, CancellationToken ct)
    {
        ...
        foreach (var stage in command.Stages)
        {
            ...
            if (!stageCodes.Add(stage.Code.Trim())) return Validation("Stage codes must be unique.");
            if (string.IsNullOrWhiteSpace(stage.Code) || string.IsNullOrWhiteSpace(stage.Name))
                return Validation("Stage code and name are required.");
            if (stage.LineNo < 1) return Validation("Stage line number must be positive.");
            if (stage.PlannedDurationMinutes < 0) return Validation("Stage duration cannot be negative.");
            if (stage.StageTemplateId is not null
                && !await db.TechnologyStageTemplates.AnyAsync(x => x.Id == stage.StageTemplateId && x.IsActive, ct))
                return Validation("Stage template must be active.");
            if (stage.DepartmentId is not null
                && !await db.Departments.AnyAsync(x => x.Id == stage.DepartmentId && x.IsActive, ct))
        ...
        var graphValidation = ValidateStageGraph(command.StageLinks, stageIds);
        return graphValidation is null ? null : Validation(graphValidation);
    }

    private static string? ValidateStageGraph(
        IReadOnlyCollection<SaveCatalogTechnologyStageLinkCommand> links, HashSet<Guid> stageIds)
    {
        var adjacency = new Dictionary<Guid, List<Guid>>();
        foreach (var link in links)
        {
            if (!stageIds.Contains(link.FromStageId) || !stageIds.Contains(link.ToStageId))
                return "Stage links must reference stages from the same technology.";
            if (link.FromStageId == link.ToStageId) return "Stage cannot depend on itself.";
            if (!Enum.IsDefined(link.LinkType)) return "Stage link type is invalid.";
            if (link.LagMinutes < 0) return "Stage link lag cannot be negative.";
            adjacency.TryAdd(link.FromStageId, []);
            adjacency[link.FromStageId].Add(link.ToStageId);
        }
    ...
    {
        foreach (var input in command.Stages.OrderBy(x => x.LineNo))
        {
        ...
        technology.StageLinks.AddRange(command.StageLinks.Select(x => new CatalogTechnologyStageLink
        {
    ...
    private static CatalogTechnologyDetails Details(CatalogTechnology x) => new(
        ...
        x.Stages.OrderBy(stage => stage.LineNo).Select(Stage).ToArray(),
        x.StageLinks.OrderBy(link => link.FromStageId).ThenBy(link => link.ToStageId).Select(Link).ToArray(),
        Convert.ToBase64String(x.RowVersion));

    private static CatalogTechnologyStageDto Stage(CatalogTechnologyStage x) => new(
        x.Id, x.StageTemplateId, x.StageTemplate?.Name, x.Code, x.Name, x.LineNo,
        x.PlannedDurationMinutes, x.DepartmentId, x.Department?.Name, x.EquipmentId,
    ...
    private static CatalogTechnologyStageLinkDto Link(CatalogTechnologyStageLink x) =>
        new(x.Id, x.FromStageId, x.ToStageId, x.LinkType, x.LagMinutes);
