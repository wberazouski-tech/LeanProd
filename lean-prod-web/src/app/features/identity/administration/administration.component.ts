...
@Component({ selector: 'app-administration', standalone: true, imports: [CommonModule, ReactiveFormsModule, TranslocoPipe, PageStateComponent, TableActionsComponent], templateUrl: './administration.component.html', styleUrl: './administration.component.css' })
export class AdministrationComponent implements OnInit {
  ...
  totalCount = 0; loading = false; message = ''; state: PageState = 'loading';
  ...
  load(): void { this.loading = true; this.state = 'loading'; const v = this.filters.getRawValue(); this.api.list(1, v.search.trim(), v.isActive, v.role).subscribe({ next: x => { this.applyPage(x); this.loading = false; this.state = x.items.length ? 'ready' : 'empty'; }, error: () => { this.loading = false; this.state = 'error'; } }); }
  ...
  editSelected(): void { if (this.selected) this.select(this.selected); }
  copySelected(): void { if (!this.selected) return; const x = this.selected; this.selected = undefined; this.form.reset({ email: '', displayName: x.displayName, temporaryPassword: '', preferredLanguage: x.preferredLanguage, defaultDepartmentId: x.defaultDepartmentId ?? '', defaultStorageLocationId: x.defaultStorageLocationId ?? '', roles: [...x.roles] }); }
  ...
  setActive(active: boolean): void { if (!this.selected) return; this.api.setActive(this.selected.id, active).subscribe(() => { this.message = this.translate.translate(active ? 'admin.activated' : 'admin.deactivated'); this.load(); this.select({ ...this.selected!, isActive: active }); }); }
  ...
  private saved(user: UserDetails): void { this.selected = user; this.message = this.translate.translate('admin.saved'); this.load(); }
  private applyPage(result: { items: UserSummary[]; totalCount: number }): void { this.users = result.items; this.totalCount = result.totalCount; }
}
