...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, TranslocoPipe, PageStateComponent, TableActionsComponent],
...
export class UnitsOfMeasureComponent implements OnInit {
  ...
  load(): void { this.state = 'loading'; const f = this.filters.getRawValue(); this.api.units(f.search.trim(), f.isActive, this.language.current()).subscribe({ next: x => { this.items = x.items; this.total = x.totalCount; this.state = x.items.length ? 'ready' : 'empty'; }, error: () => this.state = 'error' }); }
  ...
  setActive(active: boolean): void { if (!this.selected) return; this.api.setUnitActive(this.selected.id, active).subscribe(() => { this.selected = { ...this.selected!, isActive: active }; this.load(); this.loadUnitOptions(); }); }
